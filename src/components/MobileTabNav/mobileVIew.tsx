...
const MobileTabNavigation: React.FC<MobileTabNavigationProps> = ({ tabs, position, showOutlet, setIsSidebarOpen }) => {
  ...
  useEffect(() => {
    ...
    function offsetMenuBorder(element: HTMLElement | null, menuBorder: HTMLElement | null) {
      if (!element || !menu || !menuBorder) return;
    ...
    return () => {
      menuItems.forEach((item) => {
        item.removeEventListener("click", () => clickItem(item as HTMLElement, 0));
      });
    };
  }, [showOutlet, tabs]);
