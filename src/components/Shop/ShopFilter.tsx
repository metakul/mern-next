...
export default function ShopFilter({ setProducts, products = products1 }:any) {
  ...
  const [selectedAvailabilities, setSelectedAvailabilities] = useState<{ id: number; isAvailable: boolean; text: string; count: number; }[]>([]);
  const handleSelectAvailabilities = (availability: { id: number; isAvailable: boolean; text: string; count: number; }) => {
  ...
  useEffect(() => {
    ...
    filteredArrays = [
      ...
      [
        ...products.filter(
          (elm: { price: number; }) => elm.price >= price[0] && elm.price <= price[1]
        ),
      ],
    ];
    ...
    if (selectedColors.length) {
      filteredArrays = [
        ...filteredArrays,
        [
          ...products.filter((elm: { colors: any[]; }) =>
            elm.colors
              ?.map((el2) => el2.name)
    ...
    if (selectedBrands.length) {
      filteredArrays = [
        ...filteredArrays,
        [...products.filter((elm: { brand: string; }) => selectedBrands.includes(elm.brand))],
      ];
    }
    ...
    if (selectedAvailabilities.length) {
      filteredArrays = [
        ...filteredArrays,
        [
          ...products.filter((elm: { isAvailable: boolean; }) =>
    ...
    const commonItems = products.filter((item: any) =>
                  ...
                  {brands.map((brand) => (
                    ...
                    >
                      ...
                      <label className="label">
                        <span>{brand}</span>&nbsp;
                        <span>
                          ({products.filter((elm: { brand: string; }) => elm.brand == brand).length}
                          )
                        </span>
                      </label>
                    </li>
                    ...
                    >
                      ...
                      <label className="label">
                        ...
                        <span>
                          (
                          {
                            products.filter((el: { sizes: string | string[]; }) => el.sizes?.includes(elm))
                              .length
                          }
                          )
                        </span>
